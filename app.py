    ...
    for student, choices in student_choices.items():
        if len(choices) >= 2:
            for spec1, spec2 in itertools.combinations(choices, 2):
                ...
                for g1 in groups1:
                    for g2 in groups2: conflicts[g1].add(g2); conflicts[g2].add(g1)
...
def evaluate_solution_performance(student_choices, alignments, max_capacity):
    ...
    all_groups, rosters = list(group_to_alignment_map.keys()), {group: [] for group in list(group_to_alignment_map.keys())}
...
def display_solution(solution, index, student_choices, cned_assign, isolated_assign):
    ...
    num_cned = len(cned_assign)
    num_isolated = len(isolated_assign)
    ...
    with st.container(border=True):
        ...
        if solution.get('isolated_groups'):
            with st.container(border=True):
                ...
                with st.expander("Voir la liste des élèves concernés"):
                    st.write(sorted(student_list))
...
if uploaded_file:
    ...
    st.header("Analyse Initiale & Stratégie d'Externalisation (CNED)")
    initial_counts = Counter([spec for choices in original_student_choices.values() for spec in choices])
    
    with st.expander("Cliquez pour voir l'analyse détaillée des effectifs et des groupes", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
    ...
    if col_btn1.button("Trouver la MEILLEURE Combinaison", type="primary", use_container_width=True):
        with st.spinner(f"Analyse des conflits et recherche de la meilleure solution..."):
            ...
            if best_solution:
                ...
                best_solution['cned_assign'] = cned_assign # Store for display
                best_solution['isolated_assign'] = isolated_assign # Store for display
